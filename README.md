...
只有 `allowed: true` 的使用者可上傳圖片與執行 OCR。

**自動建檔（免註冊）**：`n8n_easan_operator_permission.json` 採 upsert——查無此 `userId` 時自動建立一筆「待審核」人員（寫入 LINE userId／顯示名稱／首次進入時間／角色＝操作員），管理員即可在後台「待審核人員」檢視審核開通；已存在則更新「最後使用時間」。回傳含 `allowed / role / features / status`。

**後端防繞過**：`n8n_easan_html_ocr_notion_nodes.json` 的 OCR webhook 在辨識前先查同一份名單，未開通或未開放 `OCR補料單` 直接回 403、不跑 Gemini，並記一筆「待授權」批次。

人員權限資料庫 `374b3ad1d1cd80c6a923fd022abc0305` 欄位：

- `名稱`(title)、`LINE userId`、`LINE顯示名稱`
- `狀態`：select（待審核／已開通／停用）
- `角色`：select（操作員／管理員）
- `可使用功能`：multi-select（`OCR補料單`／`報表查看`／`權限管理`）
- `首次進入時間`／`開通時間`／`最後使用時間`、`開通人員`、`備註`

> 所有 Notion 讀取節點皆使用 `simple:false` + 自寫 propVal 解析原生 properties（避免簡化輸出 key 不一致導致欄位讀不到）。
...
`n8n_easan_admin_permissions.json` 使用同一份人員權限資料庫：列出人員（依待審核→已開通→停用排序）、更新 `狀態`／`角色`／`可使用功能`（完整多選，不再只切 OCR），並於開通時寫入 `開通時間`／`開通人員`。目前 workflow 使用固定管理密碼 `gundam`。

**管理頁權限控管（密碼進入＋功能顯隱）**：管理頁以密碼登入後，再透過 LIFF 身分查該人的 `可使用功能`；只有具備 `權限管理` 者才會顯示「人員權限」分頁（桌機無 LIFF 身分時以密碼為準）。
