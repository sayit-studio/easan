...
`n8n_easan_admin_permissions.json` 使用同一個內部人員權限資料庫，可列出人員、更新 `狀態`，並切換 `可使用功能` 是否包含 `OCR補料單`。匯入後請在 n8n 設定 `EASAN_ADMIN_PASSWORD` 環境變數；若未設定，workflow 內預設為 `CHANGE_ME`，上線前必須替換。
